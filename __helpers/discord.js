...
module.exports = (type, discordId, userName, code) => {
  return new Promise((resolve, reject) => {
    if (
      ...
      return reject(
        new Error("Invalid type. Must be either get or link or confirm")
      );
    }
    ...
    switch (type.toLowerCase()) {
      ...
      case "link":
        if (!userName) {
          return reject(
            new Error("You need to include your username to link.")
          );
        }
        ...
        fetch(`${discordEndpoint}/${type}?${queryString}`)
          ...
          .then((data) => {
            ...
            if (
              ...
              data.error
                .toLowerCase()
                .includes("profile details could not be retrieved")
            ) {
