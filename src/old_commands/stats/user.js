    ...
    try {
      ...
      if (!args[0]) {
        ...
        if (discordData === false) {
          return message.channel.send(
            "Please include a username or link your account via `k!link`."
          );
        }
      }
      const playerUsername = args.join(" ")
        ? args.join(" ").trim()
        : `${discordData.name}-${discordData.discriminator}`;
      ...
      const userEmbed = new MessageEmbed()
        ...
        .setAuthor(
          ...
          `${data.avatarSrc}`,
          `${profileUrl}/${data.name}-${data.discriminator}`
        )
